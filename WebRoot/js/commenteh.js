...
jQuery.fn.rater	= function(options) {
	...
	content.find('.rater-star-item').mouseover(function() {
		...
		if (typeof settings.title_format == 'function') {
			...
			var uids=$("#odersids").val();
			$("#sleonilsn").html("");
			var schtml="";
			var sumgurl="";
			var utext="";
			$.getJSON("/peiyin//ehvoicescores.action",{"oderid":uids,"scoes":current_value},function(data){
				 if(data.jsonscoes=='1'){
					 sumgurl="p1.png";
					 utext="Very dissatisfied";
				 }
				 if(data.jsonscoes=='2'){
					 sumgurl="p2.png";
					 utext="Not satisfied";
				 ...
				 if(data.jsonscoes=='5'){
					 sumgurl="p5.png";
					 utext="Very satisfied";
				 }
				 schtml="<div style='margin-top: 7px;'>"
	                    +"<div style='float: left;'>"
	                    +"<img src='images/imgspf/"+sumgurl+"' width='122' height='27'/>"
	                    +"</div>"
	                    +"<div style='float: left; margin-top: 5px; margin-left: 2px;'>"
	                    +"<samp style='font-size: 12px; font-weight: bold;color: #F60;'>&nbsp;&nbsp;"+data.jsonscoes+"&nbspScore&nbsp;"+utext+"</samp>"
	                    +"</div>"
	                    +"</div>";
				 $("#sleonilsn").html(schtml);
			});
...
$(function(){
	...
	info_format	: function(value) {
		...
		switch (value) {
			case 1 : 
				info	= '<div class="info-box">1score&nbsp;Very dissatisfied<div>Very poor, too disappointing!</div></div>';
				break;
			case 2 : 
				info	= '<div class="info-box">2score&nbsp;Not satisfied<div>Can not meet the requirements.</div></div>';
				break;
			case 3 : 
				info	= '<div class="info-box">3score&nbsp;general<div>Feeling in general.</div></div>';
				break;
			case 4 : 
				info	= '<div class="info-box">4score&nbsp;satisfaction<div>Meet my expectations.</div></div>';
				break;
			case 5 : 
				info	= '<div class="info-box">5score&nbsp;Very satisfied<div>I am very satisfied!</div></div>';
				break;
			default :
				info = value;
				break;
