...
jQuery.fn.rater	= function(options) {
	...
	content.find('.rater-star-item').mouseover(function() {
		...
		if (typeof settings.title_format == 'function') {
			...
			var sumgurl="";
			var utext="";
			$.getJSON("/peiyin//voicescores.action",{"oderid":uids,"scoes":current_value},function(data){
				 if(data.jsonscoes=='1'){
					 sumgurl="p1.png";
					 utext="很不满意";
				 }
				 ...
				 schtml="<div style='margin-top: 7px;'>"
	                    ...
	                    +"<samp style='font-size: 12px; font-weight: bold;color: #F60;'>&nbsp;&nbsp;"+data.jsonscoes+"分&nbsp;"+utext+"</samp>"
	                    +"</div>"
	                    +"</div>";
...
$(function(){
	...
	info_format	: function(value) {
		...
		switch (value) {
			case 1 : 
				info	= '<div class="info-box">1分&nbsp;很不满意<div>音色和质量都非常差，太令人失望了！</div></div>';
				break;
			case 2 : 
				info	= '<div class="info-box">2分&nbsp;不满意<div>音色和质量不好，不能满足要求。</div></div>';
				break;
			case 3 : 
				info	= '<div class="info-box">3分&nbsp;一般<div>音色和质量感觉一般。</div></div>';
				break;
			case 4 : 
				info	= '<div class="info-box">4分&nbsp;满意<div>音色和质量都比较满意，符合我的期望。</div></div>';
				break;
			case 5 : 
				info	= '<div class="info-box">5分&nbsp;非常满意<div>我很喜欢！音色和质量都很满意，太棒了！</div></div>';
				break;
			default :
				info = value;
				break;
