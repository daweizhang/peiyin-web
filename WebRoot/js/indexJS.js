...
$(function() {
	$("#renwufangs").submit(function() {
		...
		if(laxdf!=""){
			if(xwksr!=-1){
				$("#updatejbzlrwf").html("×请认真填写您的联系电话,不能有空格!");
				flag = false;
			}
		}
		
		if (laxdf == "") {
			$("#updatejbzlrwf").html("×请认真填写您的联系电话,后期为主要联络工具!");
			flag = false;
		}
		// 地区
		var dq = $("#klnklvnse").val();
		if (dq != null) {
			...
			var diqf = dq.substring(dq.length - 2, dq.length);
			if (ids == -1) {
				$("#updatejbzlrwf").html("×地区格式不正确,请填写所在城市!");
				flag = false;
			}
			if (dq == "国家") {
				$("#updatejbzlrwf").html("×地区格式不正确,不能有国家或省份字样!");
				flag = false;
			}
			if (diqs == "国家") {
				$("#updatejbzlrwf").html("×地区格式不正确,不能有国家或省份字样!");
		...
		if (dq == "") {
			$("#updatejbzlrwf").html("×地区不能为空!");
			flag = false;
		}
		// 性别
		var sexd = $("#clklieshow").val();
		if (sexd == "") {
			$("#updatejbzlrwf").html("×性别不能为空!");
			flag = false;
		}
			...
			.submit(
					function() {
						...
						if (zsgsw.length >= 100000) {
							$("#lwnlosiockdfbrw").html("×正式稿文字若大于十万字,请联系客服人员!");
							flag = false;
						}
						if (zsgsw.length <= 1) {
							$("#lwnlosiockdfbrw").html("×正式稿文字数不能小于1!");
							flag = false;
						}
						if (!regd.test(zsgsw)) {
							$("#lwnlosiockdfbrw").html("×正式稿文字数只能为数字,不能有小数点!");
							flag = false;
						}

						if (zsgsw == "") {
							$("#lwnlosiockdfbrw")
									.html("×正式稿文字数不能为空,且根据实际情况而定!");
							flag = false;
						}
                        */
						// 正式稿文文件
						...
						// alert(lsnls);
						if (lsnls != "txt" && lsnls != "TXT" && lsnls != "rtf"
								&& lsnls != "RTF" && lsnls != "doc"
								&& lsnls != "DOC" && lsnls != "xls"
							...
							$("#lwnlosiockdfbrw")
									.html(
											"×正式试音稿文格式只能为(txt,rtf,doc,xls,ppt,html,wpd,xlsx,pdf)!");
							flag = false;
						}
						if (zsgwfle == "") {
							$("#lwnlosiockdfbrw").html("×正式稿文文件不能为空!");
							flag = false;
						}
						...
						if (filsxx != "mp3" && filsxx != "MP3"
							...
							$("#lwnlosiockdfbrw")
									.html(
											"×背景音乐文件格式只能为(MP3,OGG,CD,WMA,WAV,MP3PRO,RM,REAL,APE,MODULE,MIDI,VQF,ASF)格式!");
							flag = false;
						}
						}
						/*if (files == "") {
							$("#lwnlosiockdfbrw").html(
									"×背景音乐不能为空!如不需要,可在任务描述里面说明,付款成功后也可联系客服人员!");
							flag = false;
						}
*/
						// 试音稿文
						...
						if (gws.length > 250) {
							$("#lwnlosiockdfbrw").html("×试音稿文不能大于500字!");
							flag = false;
						}
						if (gws.length < 5) {
							$("#lwnlosiockdfbrw").html("×试音稿文不能小于5字!");
							flag = false;
						}
						if (gws == "") {
							$("#lwnlosiockdfbrw").html("×试音稿文不能为空!");
							flag = false;
						}
						...
						if (rwkouy == "0") {
							$("#lwnlosiockdfbrw").html("×请选择任务口音,可以不限!");
							flag = false;
						}
						// 任务口音类型
						var flasegs = $("#peiywjfge").val();
						if (flasegs == "0") {
							$("#lwnlosiockdfbrw").html("×请选择配音最后交付格式,可以不限!");
							flag = false;
						}
						...
						var sexs = $("#yqusex").val();
						if (sexs == "0") {
							$("#lwnlosiockdfbrw").html("×请选择任务要求性别!");
							flag = false;
						}
						...
						if (lytyope == "0") {
							$("#lwnlosiockdfbrw").html("×请选择配音语言!");
							flag = false;
						}
						// 任务类型
						var usndtupe = $("#rwleixss").val();
						if (usndtupe == "0") {
							$("#lwnlosiockdfbrw").html("×请选择任务类型!");
							flag = false;
						}
						...
						if (ysfy >= 10000) {
							$("#lwnlosiockdfbrw").html("×如预算价格大于一万,请直接联系客服人员!");
							flag = false;
						}
						if(ysfy<300){
							$("#lwnlosiockdfbrw").html("×预算价格不能小于300RMB!");
							flag = false;
						}
						if (idsre != -1) {
							$("#lwnlosiockdfbrw").html("×预算费不能有小数点!");
							$("#yusuanfeiyong").val("");
							flag = false;
						}
                          
						if (!reg.test(ysfy)) {
							$("#lwnlosiockdfbrw").html("×预算费用只能为数字且不能有小数点!");
							flag = false;
						}
						if (ysfy.length <= 1) {
							$("#rwbjslcas").html("×预算价位不能小于1!");
							flag = false;
						}
						if (ysfy == "") {
							$("#lwnlosiockdfbrw").html("×预算费用不能为空!");
							flag = false;
						}
						...
						if (rwfs != "") {
							...
							if (rwfs.length < 5) {
								$("#lwnlosiockdfbrw").html("×标题不能小于5位!");
								flag = false;
							}
							if (rwfs.length > 45) {
								$("#lwnlosiockdfbrw").html("×标题不能大于45位!");
								flag = false;
							}
						}
						if (rwfs == "") {
							$("#lwnlosiockdfbrw").html("×标题不能为空!");
							flag = false;
						}
						// 判断任务方的电话填写了没
	...
	$("#tisrwbjfrom")
			.submit(
					function() {
						...
						if (shijbgw.length > 250) {
							$("#rwbjslcas").html("×试音稿文不能大于500字!");
							flag = false;
						}
						//xiaofeng  requirement delete this logic
						/*
						if (shijbgw.length < 50) {
							$("#rwbjslcas").html("×试音稿文不能小于50字!");
							flag = false;
						}
						if (shijbgw == "") {
							$("#rwbjslcas").html("×试音稿文不能为空!");
							flag = false;
						} */
						...
						if (ysjws >= 10000) {
							$("#rwbjslcas").html("×如预算价格大于一万,请直接联系客服人员!");
							flag = false;
						}

						if (!regfy.test(ysjws)) {
							$("#rwbjslcas").html("×预算费用只能为数字且不能有小数点!");
							flag = false;
						}
						if (ysjws.length <= 1) {
							$("#rwbjslcas").html("×预算价位不能小于1!");
						...
						if (zsgstt.length >= 100000) {
							$("#rwbjslcas").html("×正式稿文字若大于十万字,请联系客服人员!");
							flag = false;
						}
						
						// delete, xiaofeng (PM) requirement
						/*if (zsgstt.length <= 1) {
							$("#rwbjslcas").html("×正式稿文字数不能小于1!");
							flag = false;
						}
						if (!regdf.test(zsgstt)) {
							$("#rwbjslcas").html("×正式稿文字数只能为数字,不能有小数点!");
							flag = false;
						}
                        */
						 
						/*if (zsgstt == "") {
							$("#rwbjslcas").html("×正式稿文字数不能为空,且根据实际情况而定!");
							flag = false;
						} */
						...
						if (bjName != "") {
							if (bjName.length < 5) {
								$("#rwbjslcas").html("×标题不能小于5位!");
								flag = false;
							}
							if (bjName.length > 45) {
								$("#rwbjslcas").html("×标题不能大于45位!");
								flag = false;
							}
						}
						if (bjName == "") {
							$("#rwbjslcas").html("任务标题不能为空!");
							flag = false;
						}
